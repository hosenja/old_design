...
class SignUpVC: UIViewController, UITextFieldDelegate, UIPickerViewDataSource, CountryPickerViewDelegate, UIPickerViewDelegate, CountryPickerViewDataSource, FBSDKLoginButtonDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        SwiftEventBus.onMainThread(self, name: "UpdatePhone") { result in
           
            self.phoneLbl.text  = result!.object as? String
...
//        countyCodePickerView.font =  UIFont(name: "Arial", size: 14)!
        ...
        phoneView.addTapGestureRecognizer {
        ...
        if self.phone != "no value"{
            phoneLbl.text = phone
           // phoneView.isMultipleTouchEnabled = false
        }
        phoneTf.isEnabled = false
        phoneView.addTapGestureRecognizer {
            self.view.endEditing(true)
            MyVriables.currentPhoneNumber = self.phoneLbl.text
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let controller = storyboard.instantiateViewController(withIdentifier: "EditPhoneNumberModal")
                ...
                DispatchQueue.main.async {
                    if self.isValidPhone(phone: self.contryCodeString+self.phoneLbl.text!)
                    {
                        ...
                        if self.contryCodeString == "+972" {
                            if self.phoneLbl.text!.count > 4 && self.phoneTf.text![0...0] == "0" {
