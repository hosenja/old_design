...
extension String {
    ...
    func makeAColl() {
        if isValid(regex: .phone) {
            ...
            let snackbar = TTGSnackbar(message: "There is problem with connecting to the required number \((MyVriables.currentGroup?.phone)!)", duration: .middle)
...
, CLLocationManagerDelegate,SwiftShareBubblesDelegate, MFMailComposeViewControllerDelegate {
    var groupsApp: [String] = []
    var groupsLabel: [String] = []
//    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
//        return groupsApp.count
//    }
    ...
    @IBOutlet weak var allPricessView: UIView!
    ...
    fileprivate func updateCell(_ cell: GroupAppCollectionViewCell) {
        if (self.singleGroup?.role) == nil
        {
            cell.groupAppView.alpha = 0.3
    ...
    fileprivate func updateAlpha() {
        ...
        {
            groupsApp.append("payments")
            groupsLabel.append("Booking")
        }
        if MyVriables.currentGroup?.group_conditions != nil {
            groupsApp.append("TermsTool")
            groupsLabel.append("Terms")
        }
        if(self.singleGroup?.group_tools?.arrival_confirmation!)! == true
        {
            groupsApp.append("arrivle_confirmation")
            groupsLabel.append("Arrival Confirmation")
        }
        if (self.singleGroup?.group_tools?.chat!)! == true
        {
            groupsApp.append("chat")
            groupsLabel.append("Chat")
        }
//        if (self.singleGroup?.group_tools?.members!)! == true
//        {
//            groupsApp.append("Members-1")
//            groupsLabel.append("Members")
//        }
//        
        
        if (self.singleGroup?.group_tools?.map!)! == true
        {
            groupsApp.append("mapp")
            groupsLabel.append("Map")
        }
        if (self.singleGroup?.group_tools?.documents!)! == true
        {
            groupsApp.append("docs")
            groupsLabel.append("Documents")
        }
        //   self.docsView.alpha = 0.3
        if (self.singleGroup?.group_tools?.checklist!)! == true
        {
            groupsApp.append("check list")
            groupsLabel.append("Check list")
        }
//            groupAppsCollectionView.reloadData()
    }
//
//    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
//         let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "GroupAppCollectionViewCell", for: indexPath) as! GroupAppCollectionViewCell
//        cell.contentView.isUserInteractionEnabled = false
//
//        cell.groupAppImage.image = UIImage(named: groupsApp[indexPath.row])
//        cell.groupAppLbl.text = groupsLabel[indexPath.row]
//        if groupsLabel[indexPath.row] == "Booking" {
//
//                    cell.alpha = 1
//                    cell.layer.cornerRadius = 10
//                    // border
//                    cell.layer.borderWidth = 0.5
//                    cell.layer.borderColor = Colors.PrimaryColor.cgColor
//
//                    // shadow
...
//                    cell.backgroundColor = Colors.paymentsBacground
//
//
//        }else {
...
//    }
    fileprivate func extractedFunc() {
        let snackbar = TTGSnackbar(message: "Please join the group in order to use the group app.", duration: .middle)
        snackbar.icon = UIImage(named: "AppIcon")
        snackbar.show()
    }
//    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
//        print("Im clicked in collection view")
//        switch groupsLabel[indexPath.row] {
//        case "Members":
//            if (self.singleGroup?.group_tools?.members!)! == true
//            {
...
//    }
    ...
    @IBOutlet weak var footerHight3: NSLayoutConstraint!
    @IBOutlet weak var footerHight2: NSLayoutConstraint!
    @IBOutlet weak var footerHight: NSLayoutConstraint!
    ...
    // time for regstrtion
    @IBOutlet weak var avalebleDatesView: UIView!
    ...
    @IBOutlet weak var bookNowBtt: UIButton!
    @IBOutlet weak var callNowBtt: UIButton!
    @IBAction func callNowBt(_ sender: Any) {
        
        setCheckTrue(type: "call_now", groupID: (MyVriables.currentGroup?.id)!)
        (MyVriables.currentGroup?.phone)!.makeAColl()
    }
    @IBAction func bookNowBt(_ sender: Any) {
        setCheckTrue(type: "book_now", groupID: (MyVriables.currentGroup?.id)!)
        bookNowFunc()
    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        avalebleDatesView.addTapGestureRecognizer {
            self.bookNowFunc()
        }
     
        let defaults12 = UserDefaults.standard
        let currentCurrency = defaults12.string(forKey: "currentCurrency")
        
        let radius : CGFloat = bookNowBtt.bounds.height/2
        // facebook
        self.bookNowBtt?.layer.cornerRadius = radius
        self.callNowBtt?.layer.cornerRadius = radius
        arrivalConfirmView.isHidden = true
        self.singleGroup  = MyVriables.currentGroup!
        if self.singleGroup?.demo != nil && (self.singleGroup?.demo)! {
            view1.isHidden = true
            view2.isHidden = true
            view3.isHidden = true
        }else {
        ...
        if singleGroup?.translations?.count != 0 {
            ...
            labelString = labelString + (singleGroup?.translations?[0].destination)! + " | "
            if MyVriables.currentGroup?.rotation != nil && (MyVriables.currentGroup?.rotation)! == "reccuring"
            {
                labelString = labelString + "\(MyVriables.currentGroup?.frequency != nil ? "\((MyVriables.currentGroup?.frequency)!.capitalizingFirstLetter()) Tour | ": "")"
                ...
                if MyVriables.currentGroup?.start_time != nil && MyVriables.currentGroup?.end_time != nil {
                    ...
                    labelString = labelString + "\(setFormat(date: (MyVriables.currentGroup?.start_time)!)) - \(setFormat(date: (MyVriables.currentGroup?.end_time)!))"
                    destntionLabel.text = labelString
                    
                    print("Label string in if reccuring \(labelString)")
                    
                }else {
                    print("Label string in else reccuring \(labelString)")
                    destntionLabel.text = labelString
                    
                }
                
            }else {
                print("Label string in else  \(labelString)")
        ...
        if MyVriables.currentGroup?.interested != nil && MyVriables.currentGroup?.interested != 0 {
            
            intrestedLbl.text = "\((MyVriables.currentGroup?.interested)!) people interested"
        }else {
            intrestedLbl.text = "25 people interested"
        }
        if (MyVriables.currentGroup?.open)! {
               self.shareGroupView.isHidden = false
        }else {
            self.shareGroupView.isHidden = true
        }
        ...
        shareGroupView.addTapGestureRecognizer(action: {
            self.bubbles?.show()
        })
...
//        bmoPageViewer.dataSource = self
       ...
       // joinView.layer.borderColor = Colors.PrimaryColor.cgColor
        ...
        if MyVriables.currentGroup!.prices != nil && (MyVriables.currentGroup!.prices!.count) > 0 {
            getCurrencyType(currencyType: currentCurrency!, city: (MyVriables.currentGroup)!)
        }else {
            
            self.allPricessView.isHidden = true
        }
        ...
        if MyVriables.currentGroup?.phone != nil{
            callNowBtt.isHidden = false
        }else {
            callNowBtt.isHidden = true
        }
        if (MyVriables.currentGroup?.group_tools?.payments!)! == false
        {
            avalebleDatesView.isHidden = true
            self.bookNowBtt.isHidden = true
            self.groupAppView.isHidden = true
            self.filterTwo.isHidden = true
            self.filterThree.isHidden = true
            self.footerHight.constant = 0
            self.footerHight2.constant = 0
            self.footerHight3.constant = 0
        }else{
            self.bookNowBtt.isHidden = false
            self.footerHight.constant = 45
            self.footerHight2.constant = 45
            self.footerHight3.constant = 45
            if (MyVriables.currentGroup?.rotation) != nil && (MyVriables.currentGroup?.rotation)! == "reccuring"{
                 avalebleDatesView.isHidden = false
                self.groupAppView.isHidden = true
                self.filterTwo.isHidden = true
...
//        }
        ...
        // Do any additional setup after loading the view.
         startTimer()
        SwiftEventBus.post("RefreshStroy")
...
//        }
    ...
    @IBOutlet weak var destntionLabel: UILabel!
    func setAlphaView()
    {
        self.singleGroup = MyVriables.currentGroup
        if (self.singleGroup?.role) == nil
    ...
    func setTripTimeDuration(startDate: String, endDate: String){
        ...
        if startMonth == endMonth 
        {
            if startDay == endDay{
                ...
                labelString = labelString + startDay+"-" + endDay+" "+endMonth
                labelString += " | \((MyVriables.currentGroup?.days)!) Days"
                destntionLabel.text = labelString
            ...
            labelString = labelString + startDay + "\(startMonth)" + "-" + endDay+" "+endMonth
            labelString += " | \((MyVriables.currentGroup?.days)!) Days"
    ...
    @IBOutlet weak var groupAppsCollectionView: UICollectionView!
    func bubblesTapped(bubbles: SwiftShareBubbles, bubbleId: Int) {
        // bbbb
        if let bubble = Bubble(rawValue: bubbleId) {
            print("\(bubble)")
            var urlGroup: String = "https://www.snapgroup.co/shareGroup.php?group=\((MyVriables.currentGroup?.id)!)"
        urlGroup = urlGroup.addingPercentEncoding(withAllowedCharacters: .urlFragmentAllowed)!
            ...
            case .whatsapp:
                ...
                if UIApplication.shared.canOpenURL(url! as URL)
                {
                    ...
                    let snackbar = TTGSnackbar(message: "Cannot send whatsapp", duration: .middle)
                    snackbar.icon = UIImage(named: "AppIcon")
                    snackbar.show()
                }
            case .google:
                ...
                mailComposerVC.setSubject("Check out ( \((MyVriables.currentGroup?.translations?[0].title)!) ) on Snapgroup")
